...
void extract_directory (uint32_t nested, FILE *pkg, uint32_t filedata_offset, uint32_t entries) {
	...
	for (uint32_t filenum = 0; filenum < entries; filenum++) {
		...
		if (directory) { // it is
			...
			fread(&dir_entries, sizeof(dir_entries), 1, pkg);
			fread(&skip_offset, sizeof(skip_offset), 1, pkg);
		...
		fread(&offset, sizeof(offset), 1, pkg);
		fread(&length, sizeof(length), 1, pkg);
...
int main (int argc, char **argv) {
	...
	fread(&filedata_offset, sizeof(filedata_offset), 1, pkg);
	...
	fread(&total_files, sizeof(total_files), 1, pkg);
	printf("(%d files total) ", total_files);
	fread(&file_count, sizeof(filedata_offset), 1, pkg);
